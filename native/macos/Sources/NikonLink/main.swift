        ...
        GeometryReader { proxy in
            ...
            if proxy.size.width > proxy.size.height {
                VStack {
                    ...
                    .padding(.leading, SpaceToken.s20)
                    .padding(.bottom, 174)
                }
            }
...
private struct ControlParameterGrid: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: SpaceToken.s8) {
            ...
            LazyVGrid(
                columns: [GridItem(.adaptive(minimum: 150), spacing: SpaceToken.s8)],
        ...
        GeometryReader { proxy in
            ScrollView {
                VStack(spacing: SpaceToken.s0) {
                    ...
                    .padding(.vertical, SpaceToken.s32)
...
private struct MonitorMacStepper: View {
    ...
    var body: some View {
        ...
        .padding(.horizontal, SpaceToken.s8).frame(height: 40)
            ...
            if store.expandedIDs.contains(branch.id) {
                ...
                if assignedPhotos.isEmpty && branch.children.isEmpty {
                    Text("拖动文件到这里")
                        .font(.system(size: TypeScale.caption))
                        .foregroundStyle(Palette.muted)
                        .padding(.leading, CGFloat(depth + 1) * SpaceToken.s16 + SpaceToken.s40)
                        .frame(height: 28)
                }
...
private struct MacLibraryBranchFileRow: View {
    ...
    var body: some View {
        ...
        .padding(.leading, CGFloat(depth) * SpaceToken.s16 + SpaceToken.s20)
        .padding(.trailing, SpaceToken.s8)
        .frame(minHeight: 56)
        .background(
        ...
        DisclosureGroup(
            ...
            VStack(alignment: .leading, spacing: SpaceToken.s12) {
                ...
                if cameraStorageSnapshot.items.isEmpty {
                    ...
                    LazyVStack(spacing: SpaceToken.s0) {
        ...
        VStack(spacing: SpaceToken.s4) {
            GeometryReader { proxy in
                ...
                ZStack {
                    ...
                    Circle().stroke(color, lineWidth: 1).padding(SpaceToken.s8)
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: SpaceToken.s20) {
                ...
                LazyVGrid(
                    columns: [
                        GridItem(.flexible(), spacing: SpaceToken.s12),
                        GridItem(.flexible(), spacing: SpaceToken.s12)
                    ],
...
private struct MyDevicesView: View {
    @ObservedObject var model: CameraModel
    private let columns = [
        GridItem(.adaptive(minimum: 300, maximum: 430), spacing: SpaceToken.s16)
    ]
...
private struct LoginView: View {
    ...
    private var codeRequired: Bool {
        auth.emailCodeMode == .required
    }

    private var codeHidden: Bool {
        auth.emailCodeMode == .notRequired
    }

    private var visibleErrorMessage: String {
        errorMessage.isEmpty ? auth.localCleanupMessage : errorMessage
    }
    ...
    private var formCard: some View {
        ...
        .shadow(color: Palette.shadow, radius: RadiusToken.r12, y: 4)
    }
    ...
    private func sendCode() {
        ...
        Task {
            ...
            do {
                try await auth.sendEmailCode(email: trimmedEmail)
                startCountdown()
