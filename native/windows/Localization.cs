...
internal static class AppLocalization
{
    ...
    private static readonly Dictionary<string, Translation> Strings =
        new(StringComparer.Ordinal)
        {
            ...
            ["搜索文件名…"] = new("Search file names…", "ファイル名を検索…"),
            ["全部"] = new("All", "すべて"),
            ["最近"] = new("Recent", "最近"),
            ["名称"] = new("Name", "名前"),
            ["{0} 个文件"] = new("{0} files", "{0} ファイル"),
            ["{0} 张照片 · {1} 个视频"] = new("{0} photos · {1} videos", "写真 {0} 枚 · 動画 {1} 件"),
            ["将 {0} 移到废纸篓？"] = new("Move “{0}” to the Recycle Bin?", "「{0}」をごみ箱に移動しますか？"),
            ["外录文件仍被当前会话引用"] = new("External recording is still referenced by the current session", "外部収録ファイルは現在のセッションで参照されています"),
            ["显示来源与工具"] = new("Show sources & tools", "ソースとツールを表示"),
            ["隐藏来源与工具"] = new("Hide sources & tools", "ソースとツールを隠す"),
            ["预览"] = new("Preview", "プレビュー"),
            ["移动到项目"] = new("Move to project", "プロジェクトに移動"),
            ["选择要移动到的项目；文件位置保持不变。"] = new(
                "Choose a project. The file stays in its current folder.",
                "移動先のプロジェクトを選択してください。ファイルの保存場所は変わりません。"),
            ["移动"] = new("Move", "移動"),
            ["导出副本"] = new("Export copy", "コピーを書き出し"),
            ["文件已移到回收站"] = new("File moved to the Recycle Bin", "ファイルをごみ箱に移動しました"),
            ["暂无匹配文件。拍摄、从相机下载或导入照片与视频后会显示在这里。"] = new(
                "No matching files. Capture, download from a camera, or import photos and videos to see them here.",
                "一致するファイルはありません。撮影、カメラからのダウンロード、写真や動画の読み込み後にここへ表示されます。"),
