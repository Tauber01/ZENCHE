...
| Develop · 修图 | 五组专业显影参数、透明预设、原图对比、旋转翻转、比例裁切与非破坏性高质量 JPEG 副本 |
...
AI 功能采用**激活码制**：每个激活码绑定一台设备、可生成 100 张图片，由作者维护的
服务器端计数并转发模型请求；应用开源客户端不内置任何模型 API 密钥，源码可安全公开。
购买与激活码获取渠道见应用内"设置 → AI 功能激活"。
...
版本及同名 `.sha256` 校验文件。1.3.0 的交付文件命名如下：

| 平台 | 文件 | 安装说明 |
| --- | --- | --- |
| macOS Apple Silicon | `ZENCHE-1.3.0-macOS-arm64.dmg` | 拖入 Applications；社区构建为 ad-hoc 签名，未公证 |
| Android | `ZENCHE-1.3.0-android.apk` | 允许侧载后安装；当前使用调试证书签名 |
| Windows x64 | `ZENCHE-1.3.0-Windows-x64-Setup.exe` | 推荐安装程序；当前未使用商业代码签名证书 |
| Windows x64 便携版 | `ZENCHE-1.3.0-Windows-x64.zip` | 完整解压后运行，不要单独移动 `libusb-1.0.dll` |
| HarmonyOS | `ZENCHE-1.3.0-HarmonyOS.hap` | 真机安装前需要有效的开发者签名与 Profile |
| iOS / iPadOS | `ZENCHE-1.3.0-ios-unsigned.ipa` | CI 验证产物；必须重新签名，不能直接安装 |
...
```sh
shasum -a 256 -c ZENCHE-1.3.0-macOS-arm64.dmg.sha256
```

Windows PowerShell：

```powershell
Get-FileHash .\ZENCHE-1.3.0-Windows-x64-Setup.exe -Algorithm SHA256
```
...
| Develop | Five professional adjustment groups, transparent presets, before/after comparison, rotation, flipping, aspect-ratio cropping, and non-destructive high-quality JPEG copies |
...
AI features use an **activation-code model**: each code binds to one device and enables
100 generations, counted and proxied by the author's server. The open-source clients
never embed any model API key, so the source stays safe to publish.
See "Settings → AI Activation" in-app for purchase details.
...
[GitHub Releases](https://github.com/Tauber01/ZENCHE/releases). Version 1.3.0
uses the following delivery names:

| Platform | File | Installation note |
| --- | --- | --- |
| macOS Apple Silicon | `ZENCHE-1.3.0-macOS-arm64.dmg` | Drag to Applications; community build is ad-hoc signed and not notarized |
| Android | `ZENCHE-1.3.0-android.apk` | Sideloading required; currently signed with a debug certificate |
| Windows x64 | `ZENCHE-1.3.0-Windows-x64-Setup.exe` | Recommended installer; no commercial code-signing certificate |
| Windows x64 portable | `ZENCHE-1.3.0-Windows-x64.zip` | Extract completely; keep `libusb-1.0.dll` beside the executable |
| HarmonyOS | `ZENCHE-1.3.0-HarmonyOS.hap` | A valid developer signature and Profile are required for device installation |
| iOS / iPadOS | `ZENCHE-1.3.0-ios-unsigned.ipa` | CI validation artifact; it must be signed before installation |
...
| Develop · 現像 | 5 グループのプロ調整、透明なプリセット、補正前後比較、回転・反転、縦横比クロップ、非破壊の高品質 JPEG コピー |
...
AI 機能は**アクティベーションコード方式**です。各コードは 1 台のデバイスに紐づき、
100 回の生成を可能にします。作者のサーバーで回数を管理し、モデルリクエストを中継します。
オープンソースのクライアントにはモデル API キーを一切埋め込まないため、ソースを安全に公開できます。
購入方法はアプリ内「設定 → AI 機能のアクティベーション」を参照してください。
...
パッケージと同名の `.sha256` ファイルをダウンロードしてください。1.3.0 の
配布ファイル名は次のとおりです。

| プラットフォーム | ファイル | インストール上の注意 |
| --- | --- | --- |
| macOS Apple Silicon | `ZENCHE-1.3.0-macOS-arm64.dmg` | Applications へドラッグ。コミュニティ版は ad-hoc 署名で未公証 |
| Android | `ZENCHE-1.3.0-android.apk` | サイドロードが必要。現在はデバッグ証明書で署名 |
| Windows x64 | `ZENCHE-1.3.0-Windows-x64-Setup.exe` | 推奨インストーラー。商用コード署名証明書は未使用 |
| Windows x64 ポータブル | `ZENCHE-1.3.0-Windows-x64.zip` | 完全に展開し、`libusb-1.0.dll` を実行ファイルと同じ場所に保持 |
| HarmonyOS | `ZENCHE-1.3.0-HarmonyOS.hap` | 実機インストールには有効な開発者署名と Profile が必要 |
| iOS / iPadOS | `ZENCHE-1.3.0-ios-unsigned.ipa` | CI 検証用。インストール前に署名が必要 |
