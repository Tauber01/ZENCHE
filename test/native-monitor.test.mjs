...
test('native monitor surfaces use native RGB and audio scope plots', () => {
  const ios = read('native/ios/NikonLink/Views/RootView.swift');
  ...
  assert.match(ios, /RGBWaveformCard/);
  assert.match(ios, /AudioScopePlot/);
  assert.match(android, /WaveformScopeView\.RGB_PARADE/);
  assert.match(android, /WaveformScopeView\.AUDIO/);
  assert.match(harmony, /drawMonitorRgbScope/);
  assert.match(harmony, /drawAudioScope/);
  assert.match(macos, /monitorRGBWaveformCard/);
  assert.match(macos, /MacAudioScopePlot/);
  assert.match(windows, /x:Name="MonitorRgbScope"/);
  assert.match(windows, /Mode="Audio"/);
});
