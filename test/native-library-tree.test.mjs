...
test("branch organization remains available and supports persistent file drag moves", async () => {
  const [ios, android, harmony, macos, windows, windowsXaml] =
    await Promise.all([
      read("native/ios/NikonLink/Views/RootView.swift"),
      read("native/android/app/src/main/java/com/tauber/nikonlink/MainActivity.java"),
  ...
  // 1.5.14 文件库 UX 重构：全平台以「所有文件」为主，分支树降级为
  // 默认收起的项目分类；拖动归类保留且不会移动原文件。
  ...
  assert.match(macos, /显示来源与工具/);
  assert.match(macos, /branchWorkspace/);
  assert.match(windowsXaml, /x:Name="SourcesAndToolsExpander"/);
  assert.match(windowsXaml, /<TreeView/);
