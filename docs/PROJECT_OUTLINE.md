...
Windows 1.5.11 启动异常已有本地热修候选：WPF 在 `InitializeComponent()` 期间处理 XAML 默认选中项时，曝光模式、视频快门模式和共享参数处理器会过早访问尚未创建的后置控件，形成 `SetParameterAvailability → UpdateExposureAvailability → ExposureModeBox_SelectionChanged` 的空引用链。候选提交 `970f8e08edce2529750d5b29fe3aaccd53da61ac` 将三条路径统一置于初始化门禁之后，并以 3 条启动契约锁定事件连接、默认选中、控件顺序、门禁初值及完整树加载后的恢复配置；完整 `npm test` 517/517 通过。macOS 上已交叉生成本地 `1.5.11` Windows Setup/ZIP，SHA-256 分别为 `69afb3763b374005a97b6ef1da558c7dded8fa5d94c954d5aa673560fa7d5d47` 与 `37ba48fae87f3fd074a222fe214dadacd882e3d3e0383333aa98246a38917e40`。这两个同版本文件仅是本地热修候选，未替换 GitHub 资产、未进入官网清单且无 Authenticode；仍需真实 Windows 冷启动、参数交互、安装升级、卸载、驱动和 SmartScreen 验收。
