...
- **W13 账号系统候选**：五端已接入邮箱注册、登录墙、会话校验与账号绑定，原生客户端候选为 `c661f7f`；管理台“总用户”统计由 `adf310d` 增加全部注册账号计数、`9d9ce1e` 明确说明“含未验证、已禁用、未绑定设备账号”，均未推送或发布。账号密码与 session token 只允许经 `https://zenche.top/api` 访问；历史 `http://101.34.255.115:8787` AI 配置不得作为认证基址，AI endpoint 为 HTTP 时客户端不得附加账号 Bearer。AI审查 与 GPT5.6luna 已分别签发原生视觉/交互、用户可见内容的代码静态 PASS。2026-08-09 已为官网 Nginx 补齐 `/api/v1/auth/*` 与 `/api/v1/ai` 精确 HTTPS 反代，公网真实注册、登录、`me`、登出及无效激活 JSON 路由冒烟通过；临时账号已禁用留档。有效激活码的真实 AI 生成、真机视觉/安装矩阵和五端版本化交付包仍未完成，因此 W13 客户端仍不得推送或发布；管理台统计修正也尚未部署。
