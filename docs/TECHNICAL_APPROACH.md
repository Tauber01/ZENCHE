...
- 修复代码提交为 `970f8e08edce2529750d5b29fe3aaccd53da61ac`，已于 2026-08-11T09:10:29Z 随 [v1.5.12 GitHub Latest](https://github.com/Tauber01/ZENCHE/releases/tag/v1.5.12) 发布；注释标签解引用到 `bb9aab0707a0260e6dd44667836e710d2820485f`。冻结实现与打包源码基线 `025517c179003db1790a3a5c1ffd0560ce55d39e` 的完整 `npm test` 518/518。先前沿用 1.5.11 名称的本地 Windows 热修包只用于验证构建链，本次六个安装包及侧车均由冻结候选重新生成；Windows Setup/ZIP SHA-256 分别为 `ceb97d5552c18b15cc05f054c1c7c3c1486367d1cb764ef561aeb74cc253a4fe`、`2f9d0cd10b43015c9aed943c5a8eefb37799c322eec6ab83d095fc456477ea69`，14 项聚合包 SHA-256 为 `1bdaed8f3ae6214e1f8aa3133d8e88fa5a7274a0b711e52374a30b2c38d5f2ce`。Release 为非草稿、非预发布，14 个线上附件的名称、字节数与 GitHub SHA-256 摘要均和本地一致。Windows 包继续由 macOS 交叉构建、无 Authenticode，仍需真实 Windows 安装与启动验收；官网自动更新继续保持 1.5.10 / build 37。
...
### 5.2 Sony 旧机型兼容、桌面导入与文件库数据契约（1.5.14）

- **兼容表语义**：五端注册表新增 ZV-E10、A6100、A6400、A6600 四个规范档案，A6100A/A6400A 作为描述符识别别名折叠到对应规范档案。注册表共 50 款（Nikon 20、Sony 16、Canon 14）。Sony 当前公开的 Camera Remote SDK 支持表未列出这批旧机型，因此 macOS 先走官方 SDK，明确不支持时才允许 gphoto2/PTP 兼容回退；文档只能声称“识别与兼容配置”，不能声称官方 SDK 全功能认证。
- **连接收口**：桌面外部相机命令的等待上限按“最后一次标准输出/错误输出活动”刷新，静默超时必须终止整个进程组并返回稳定错误，避免有输出的长任务被误杀，也避免子进程遗留。iOS 可信局域网桥只把 Nikon 送入 Nikon 兼容桥，Sony 档案不得再进入该路径。
- **一键导入**：macOS/Windows 原生多选器统一覆盖 JPG/JPEG、PNG、HEIF/HEIC、TIF/TIFF、NEF/NRW、ARW、CR2/CR3、MOV/MP4/M4V、AVI。每个文件流式复制到目标目录内的 `.zenche-import-*.part`，同步落盘并核验字节数后原子发布；批次可取消、输入去重、RAW+JPEG 同目录配对，成功项继续完成 XMP、双备份、SHA-256 清单和进度统计，单项失败不回滚已经提交的其他文件。
- **所有文件优先**：五端主视图从持久化文件快照生成“所有文件”，提供文件名搜索、照片/视频筛选、最近/名称排序。项目分类只保存路径到分类 ID 的元数据映射，不移动或改名原文件；移动端“项目分类”、桌面“来源与工具”默认收起。Windows 在 `RefreshPhotoList()` 时读取一次库快照，键入搜索只处理内存列表；macOS 使用 `LazyVGrid`，Windows 使用回收虚拟化。
...
- 五端以“所有文件”为主视图，支持文件名搜索、照片/视频筛选、最近/名称排序；项目分类和来源工具默认收起。
- 支持任意层级用户分支、持久化展开状态、拖拽归类、安全删除和限额缩略图；归类只更新元数据映射，不移动原文件。
- 删除分支时恢复或迁移其中素材，不能静默删除原文件。
- 删除文件使用系统废纸篓/回收站；关联备份、校验清单与共享 XMP 必须按真实历史会话收口。
- macOS/Windows 提供流式、可取消的照片和视频多选导入；移动端继续使用系统相册与文档提供器路径。
...
- `1.5.13 / build 40` 的最终发布实现冻结于 `a89e2f89416e6a70564258d9f421d58d6cdc75cd`。该提交把五端三语启动公告固定为长期有效的 GitHub Release 与官网 1.5.10 更新边界，并将产品名称统一为“专业显影 / Pro Develop / プロ現像”；三语正向匹配和旧措辞反向断言防止回归。exact HEAD 完整自动化 532/532 后，Android Debug、iOS unsigned、HarmonyOS unsigned、macOS arm64 ad-hoc 与 Windows x64 Setup/ZIP 六包均从该提交重建；六份侧车、四个 ZIP 容器、DMG、版本、签名边界和包内公告已复验。聚合包含六包、六侧车与发布说明，共 13 项、275,166,398 字节，SHA-256 `e6304e72477e9f8c64cff7b1d68ec36750eb928fb8b02faf33bfa3f576c10266`，与交付源逐字节一致。Windows 仍由 macOS 交叉构建且无 Authenticode，不得据此声称真实 Windows 启动、安装、驱动或 SmartScreen 已验收；官网生产清单继续保持 1.5.10 / build 37。逐包字节数与 SHA-256 见 `docs/releases/v1.5.13.md`。
- `v1.5.13` 注释标签解引用到 `c8fc8139e01bb2fd9b12b338c7e81a89702e3e98`，Release 于 `2026-08-12T03:33:52Z` 公开为 Latest、非草稿、非预发布。一次性冻结资产发布 run `31560420972` 先按分片、聚合包与六份侧车校验本地字节，再上传六包、六侧车、聚合包及其侧车；公开后重新下载全部 14 个资产并逐项 `cmp`，同时核对名称、字节数与 GitHub SHA-256 摘要，14/14 一致。临时桥接分支与两个触发标签已删除，常规 Release workflow 的 v1.5.13 精确跳过条件也在本次发布后移除。
...
- 50 款相机注册表、Sony 旧机型识别别名与 Android USB filter 对齐。
