...
- 全屏监看必须以实时画面为主，顶部遥测、焦点十字、工具轨、示波器和底部参数托盘只读取既有相机/监看状态。RGB 示波器使用实际直方图或画面分析数据；PTP 无音频源时只显示并标注静音基线，不生成模拟电平。
...
- 1.5.3 对应版本 `1.5.3 / build 28`。它是本地开发候选；未获得 Tauber 的独立授权前，不得推送、打标签、创建 GitHub Release 或修改生产环境。
