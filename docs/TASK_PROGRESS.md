...
> 当前开发候选：`agent/1.5.10-login-layout`，1.5.11 / build 38；连续调节、AI 空间与窄窗响应式实现/桌面包源码 `97659b2ea7a588720be041e32f736c98e8cec65c`，双审和 14 项聚合总包已封板
...
- 当前开发候选为 **1.5.11 / build 38**，位于 `agent/1.5.10-login-layout`；连续调节、AI 空间与窄窗响应式实现/桌面包打包源码为 `97659b2ea7a588720be041e32f736c98e8cec65c`。macOS DMG 与 Windows Setup/ZIP 已按该提交重建并通过同名侧车、结构及签名属性回验；Android、iOS、HarmonyOS 沿用同一候选的既有包。最终聚合包 SHA-256 为 `77a2f957c89f332e9b3afd09ad1a9e81ed26f0375d5b2201580b7644051dd15f`，14 项内容与当前六包、六侧车和两份说明逐字节一致。GitHub 公开稳定版仍是 [v1.5.3](https://github.com/Tauber01/ZENCHE/releases/tag/v1.5.3)，官网自动更新继续提供 1.5.10 / build 37；1.5.11 尚未部署、推送、打标签或创建 GitHub Release。
- W15 已完成源码增量：五端登录动作与 Apple 忙碌态、逐平台签名公告保持闭环；macOS、Windows 的主窗口与面板状态继续持久化。桌面分隔条现为拖动中实时连续调节，Windows 新增 AI 工具面板宽度，macOS AI 区释放重复导航与通用底栏占用的高度，两端预设与安全范围同步扩大。窄窗时会联动限制外层侧栏和内部面板，Windows 紧凑 AI 页优先保留选项区。完整 `npm test` 493/493 通过，两端桌面编译/打包与聚合包回验通过；原生 UI/交互终审 P0/P1/P2 均为 0，材料审查指出的聚合包、状态与英文术语问题已全部闭环。真实桌面拖拽/重启恢复与 Windows 多显示器/DPI 仍需实机验收。
...
| 登录动作清晰化与桌面工作区 | 已实现待验收 | `97659b2`；五端模式/提交与签名公告契约；Apple 忙碌态 exact 三语；macOS/Windows 连续实时分隔条、AI 空间、窄窗安全约束与布局状态；完整 493/493、桌面三包、双审和 14 项聚合包通过 | macOS/Windows 真实拖拽与重启恢复、Windows 多显示器/DPI、辅助功能实机 |
