...
- **下一步**：提交最终术语实现；对 exact HEAD 执行完整测试，从该精确提交重新生成六包及侧车并复核版本、容器、签名和 SHA-256；重新生成聚合包、回填发布材料，再推送 `main`、注释标签 `v1.5.13` 和 GitHub Release，并在线核对 14 个资产。官网自动更新继续保持 1.5.10 / build 37；真实文档提供器、同名覆盖、无空间、大文件、移动端权限、macOS Gatekeeper、公证与真实 Windows 启动/安装/驱动/SmartScreen 仍作为独立验收项。
